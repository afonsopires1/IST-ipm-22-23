...
{ 
  ...
  start_button.position(width/2 - start_button.size().width/2, 150);
  
  textFont("Arial", 15);
  textStyle(BOLD);
  fill(color(255, 255, 255));
  textAlign(LEFT);
  text(intro, 10, height/ 2   + start_button.size().height);
  
  image(img, 10, 350, 650, 100);
...
function validID()
{
  ...
  {
    alert("Ups, INVALID STUDENT NUMBER 👎\nPlease insert a valid student number (between 1000 and 200000) 😄");
	return false;
  }
}
...
function validSize()
{
  ...
  {
    alert("Ups, INVALID DISPLAY SIZE 👎\nPlease insert a valid display size (between 13 and 50).😄");
    return false;
  }
}
